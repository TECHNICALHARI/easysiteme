...
function pickPrimaryCTA(form: FormData | any) {
    const socials = form?.profile?.socials ?? form?.socials ?? {};
    if (socials?.calendly) return { label: "Book a Call", href: socials.calendly };
    const links = Array.isArray(form?.profile?.links) ? form.profile.links : Array.isArray(form?.links) ? form.links : [];
    const primary = links.find((l: any) => l?.highlighted);
    if (primary) return { label: primary.title || "Primary", href: primary.url || "#" };
    const site = links.find((l: any) => typeof l?.url === "string" && /https?:\/\//.test(l.url));
    if (site) return { label: site.title || "Visit Website", href: site.url || "#" };
    return null;
}

export default function StickyActionBar({ form }: { form: FormData | any }) {
        ...
        try {
            if (navigator && (navigator as any).share) {
                await (navigator as any).share({
                    title: form?.profile?.fullName ? `${form.profile.fullName} | myeasypage` : "Check my profile",
                    text: form?.profile?.bio ?? undefined,
                    url,
                });
            } else {
                setOpenShare(true);
    ...
    return (
        <>
            <motion.div
                ...
                className={`fixed left-1/2 -translate-x-1/2 bottom-4 z-[90] w-[95%] max-w-md sm:max-w-xl ${isOpen ? "block" : "hidden sm:block"}`}
            >
            ...
            {!hideBar && (
                <button
                    ...
                    className="sm:hidden fixed bottom-4 right-4 z-[95] rounded-full bg-[var(--color-accent)] text-white p-3 shadow-lg"
